...
import { buildPredicatePage } from '@/lib/registry/registryEntityPage';
import { allPredicates, predicateById } from '@/lib/registry/registryEntities';
...
export function generateStaticParams() {
  return allPredicates().map((p) => ({ id: p.id.toLowerCase() }));
}

export async function generateMetadata({ params }: { params: Promise<{ id: string }> }): Promise<Metadata> {
  const { id } = await params;
  const e = predicateById(id);
  if (!e) return {};
  const c = buildPredicatePage(e);
  return {
