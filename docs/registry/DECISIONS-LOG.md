...
| **D-021** | **Ordre de promotion à strates** (précise D-020 pour la place d'OCR-006) : **`{OCR-000, OCR-005}` → `OCR-006` → `{001, 002, 003, 004}` → 23 concepts.** OCR-006 promu **2ᵉ** (après la paire ratifiée, avant les 4 autres méta) | **rendue, INSCRITE — NON appliquée** | **`bfc888a`** | **—** · *ordre d'exécution de l'**étape 8** ; l'invariant d'ordre (D-020) devra le vérifier **à strates**, pas à deux — note étape 8, non construit* |
...
| **2** | **Édition OCR-001** (D-018, en place, Version **1.0.0** inchangée) — étendre la plage méta **`000–005 → 000–009`** (répare l'anomalie OCR-006 **+** réserve les 3 Records du régime : `007`,`008`,`009`) ; **+ étendre `expected_ranges`** du manifeste (artefact **distinct**) ; **+ 2 gardes** : OCR-006 ∈ plage, **cohérence plage↔manifeste** | D-015 · **D-018** · `OCR-005:33` | ⛔ non lancée — *conception rendue, à revoir* |
| **3** | **Loger les règles du régime documentaire (D-015)** — **3 nouveaux Records méta** : **`OCR-007` intégrité** (checksum), **`OCR-008` transmission** (chemin+hash), **`OCR-009` approbation** (D-016 ; décrit aussi le **lieu** `founding/`+RATIF, qui **n'est PAS un OCR** — autorité par signature, non ratifiable). Le 4ᵉ déficit (lieu) est **décrit, pas créé comme Record** | D-015 · D-016 · D-018 · D-019 | ⛔ non lancée |
