...
| **D-010** | Grain de l'artefact PROMO | **✅ RÉSOLUE le 2026-07-26 — GRAIN SOUPLE** | *(ce log)* | — · *un fichier PROMO peut déclarer PLUSIEURS Records (`declares_normative` = tableau) ; déjà supporté par le résolveur (grain-agnostique), aucun changement de code* |
...
| **D-021** | **Ordre de promotion à strates** (précise D-020 pour la place d'OCR-006) : **`{OCR-000, OCR-005}` → `OCR-006` → `{001, 002, 003, 004}` → `{007, 008, 009}` (régime, strate 2) → 23 concepts.** OCR-006 promu **2ᵉ** ; les 3 Records du régime rejoignent la **strate 2 (méta)** | **rendue, INSCRITE — NON appliquée** | **`bfc888a`** | **—** · *ordre d'exécution de l'**étape 8** ; **FAIT gravé (étape 3) : corpus 33→36, promotion 29→32** (`promotionDebt` dit 36/32) ; l'invariant d'ordre (D-020) à vérifier **à strates**. **⚠️ AMENDÉ par D-024 (2026-07-25)** : `OCR-112` **sort de la strate « 23 concepts » (→ 22)** — ordre effectif = **32 = Phase 1**, écart en extension **vide** (démontré, [[PARTITION-CONSOLIDEE-conception]]). **⚠️ À vérifier avant étape 8** : OCR-009 décrit l'approbation qui gouverne la promotion — dépendance sur sa propre promotion ? (constat pour plus tard)* |
...
| **D-024** | **OCR-112 → Phase 2** — 112 est **Phase 1** (hors `HORS_PHASE_1`, placé S4) **mais bloqué par GAP-F1-01** (supersession absente) : obstacle réel au lot courant. Plutôt que coder la supersession (lot non cadré tiré dans la promotion), 112 **rejoint 114** (même bloqueur) en **Phase 2** ; le GAP se lèvera en lot de code propre, 112+114 remonteront ensemble. **MOTIF PROPRE (décision d'espèce, D-025) :** 112 descend **uniquement** parce qu'il **partage GAP-F1-01 avec 114** — les deux se lèvent par le **même lot de code** (coder la supersession) et **remontent ensemble**. **Raison particulière à 112/114, PAS règle générale** : un GAP ouvert ne renvoie **pas** en Phase 2 par lui-même (D-025). Sans ce motif, D-024 se lirait comme la règle que D-025 écarte | **rendue + APPLIQUÉE le 2026-07-25** | **`8bb0f2b`** | **`8bb0f2b`** · *ajout à `HORS_PHASE_1` ; net avec la **correction 006** (`c82f8f7`, retrait — D-021 prime) = swap 006↔112, **Phase 1 = 32** (garde `promotionDebt` verte)* |
| **D-025** | **« GAP ouvert » n'est PAS un critère de phase** — **décision d'ESPÈCE**. Un GAP ouvert **ne renvoie pas** en Phase 2 par lui-même ; le critère général « GAP ouvert → Phase 2 » est **écarté**. Les quatre Records Phase-1-avec-GAP (**101, 110, 111, 113**) **restent en Phase 1** ; **Phase 1 reste 32**. Leur sort est renvoyé à **E** (seuil « prêt » d'un G1), désormais **BLOQUANT pour les quatre** | **rendue + INSCRITE le 2026-07-26 — sans application** (état : partition **inchangée = 32**) | **rendue le 2026-07-26** | **—** · *contrepartie : **D-024** reçoit son **motif propre** (espèce). **OCR-113** « OCR-REV à confirmer » = **propreté documentaire**, **plus** un bloqueur de partition (113 reste Phase 1). Réf. [[DOSSIER-ARBITRAGE-GAP-PHASE1]]* |
| **D-026** | **`OCR-000:49` PRIME** — la gradation de grounding **G0–G4** est un **instrument d'audit SANS rang normatif** ; sa « règle d'or » (*Normative ET non implémenté + badge « planifié »*) est **écartée**. **Tout désaccord doc↔code — GAP nommé OU G1 « code incomplet » — INTERDIT la promotion, SANS seuil** (`OCR-000:49` : « the disagreement MUST be resolved before promotion; the Registry MUST NOT paper over it »). **E est CLOSE, pas ajournée** — il n'y a **aucun seuil à définir** | **rendue + INSCRITE le 2026-07-26 — sans application** (règle de gouvernance) | **rendue le 2026-07-26** | **—** · *dossier [[DOSSIER-E-SEUIL-G1-mesure]] = la mesure qui a révélé le conflit (aboutit à aucun seuil). **Conséquence mesurée : le sous-ensemble franchissable rétrécit fortement** — [[FRANCHISSABLE-SOUS-D026-mesure]] (blocage = tout G1/G3, pas seulement les 4 GAP nommés)* |
| **D-027** | **GRAIN SOUPLE** — un fichier PROMO déclare **un OU plusieurs** Records (`declares_normative` = tableau) ; l'acte fondateur (RATIF, 2 Records) devient un **cas ordinaire**, plus une exception. La garde de l'**identifiant** se déplace du **nom de fichier** vers le **CONTENU** — ce qui **ferme le gap-2 de D-010** (id à source unique) par la garde de contenu, plus par le nom | **rendue + APPLIQUÉE le 2026-07-26** | **rendue le 2026-07-26** | **—** · *clôt le volet « grain » de D-010. Lieu `content/registry/promo/` créé **À VIDE** (manifeste + schéma + README) + garde de contenu `promoLieu.test.ts` : **R1** id déclaré existe · **R2** aucun doublon intra-fichier · **R3** aucun id promu 2× inter-fichiers · **R4** plage, cohérente au manifeste OCR (D-004) — **10/10 mutation-prouvée**. AUCUN PROMO réel écrit ; le résolveur ne voit que `RATIF-001`. Émission = étape 8* |
| **D-028** | **Voie A — OCR-000 amendé par versioning pour citer OCR-009.** Plutôt qu'une édition en place (interdite sur un Record ratifié, D-022), l'orphelin attendu OCR-009 est cité par un **amendement versionné** : `OCR-000:47` cite désormais OCR-009 (« Opus X approval (its form is defined by OCR-009) ») → la porte **« cité ≥ 1 »** est **franchie**, OCR-009 devient promouvable. **Premier amendement post-ratification** | **rendue + INSCRITE + APPLIQUÉE le 2026-07-26** | **`728e7a8`** *(OCR-000 v1.1.0)* | **`728e7a8`** — amendement versionné (v1.0.0 → v1.1.0), citation gravée ; `citations.test.ts` : orphelins = `['OCR-123']` (OCR-009 sorti de l'ensemble) |
| **D-029** | **OCR-113, correctif C1 — l'intégrité est un SHA-256 NON keyé sur la forme canonique JCS, pas un HMAC keyé.** Le texte se **contredisait** (« vérifiable par quiconque, sans faire confiance au présentateur » ⇒ digest **non keyé** ; un HMAC keyé exigerait le secret) : régime **OCR-REV**, le code fait foi (`canonical.ts:84` SHA-256 non keyé, recalculé + comparé en temps constant `ingestion:176-179`). Correctif **thorough** (plus aucune mention de HMAC keyé dans l'intégrité) ; transport hors périmètre | **rendue + INSCRITE + APPLIQUÉE + PROMUE le 2026-08-31** | **`1cbc5d6`** *(correctif C1, v1.0.0 → v1.1.0)* | **`1cbc5d6`** — correctif appliqué ; **PROMUE `9854358`** (PROMO-004, OCR-113 Draft → Normative) |
| **D-030** | **OCR-110, Voie Code (COMPOSITE).** Régime OCR-REV à deux volets : **(cardinalité) APPLIQUÉ** — « MAY reference multiple criteria » → « exactly one criterion » (`criterion_levels` = exactement une entrée), doc = code (`ingestion:184-188`, `observation_invalid` si compte ≠ 1) ; **(lien Passport 1:1) NON appliqué — SUPERSEDÉ par D-031**. La Voie Code supposait un ajout borné ; la **mesure a révélé** qu'elle ouvrait le **cycle de vie du Passport** (non construit, gelé au Sprint 1) → **rouverte en D-031** (report Phase 2). **voir D-031** | **rendue le 2026-08-31 — COMPOSITE (PAS « appliquée » tout court)** | **`c29f8e4`** *(volet cardinalité, v1.0.0 → v1.1.0)* | **cardinalité `c29f8e4`** · **lien Passport NON appliqué — superseded D-031** |
| **D-031** | **REPORTER OCR-110 en Phase 2** (Voie Code différée) — le lien Passport 1:1 (`OCR-110:115` « every accepted Evidence SHALL be linked to exactly one Passport update ») **exige le cycle de vie du Passport**, qui **n'est pas construit** : la « Passport update » **n'existe pas** (ni table ni objet), le pont WSP→`passports` est **absent** (`wsp_evidence.subject_id → profiles.opus_id`, jamais un passport), et `passports.lifecycle_stage` est **gelé à `identity_established`** (Sprint 1, étapes 2-7 inertes). Les **3 questions de conception** — evidence révoquée garde-t-elle son lien ? update sans evidence ? sujet sans passport ? — **ne sont pas tranchées**. La **lecture minimale** (bookkeeping sans avancer le cycle) serait une **coquille vide** : **refusée**. OCR-110 **gèle en Phase 2** jusqu'au chantier « cycle de vie du Passport ». Le **volet cardinalité corrigé reste acquis** (`c29f8e4`, D-030 A) | **rendue + INSCRITE le 2026-08-31 — sans application** (report ; rien construit, rien promu) | **rendue le 2026-08-31** | **—** · *chantier « cycle de vie du Passport » tracé (chip) ; 2 volets résiduels 110 (forme du payload + formule keyed-HMAC `:64`/`:122`) tracés (chip). Réf. cadrage D-030 (C). ⚠️ D-028/D-029/D-030 non encore inscrites au log — trou signalé à l'Architecte* |
...
| Réf | Objet | Statut |
|---|---|---|
| **D-010** | **Grain de l'artefact de promotion** — un artefact par Record (fin) vs un artefact multi-événements (gros) | **✅ RÉSOLUE le 2026-07-26 — GRAIN SOUPLE** : un fichier PROMO **peut déclarer PLUSIEURS Records** (`declares_normative` = tableau — ni « un par fichier » imposé, ni « un gros » imposé). **Déjà supporté par le résolveur** (grain-agnostique, `4bf8977`) ; la garde du lieu `promo/` (à créer) acceptera **1+ ids** (≠ RATIF = exactement 2). Amont levé : D-013 « sort du champ » = **projection** (champ gardé, ignoré ; étape 6) ; gap « ordre » **dissous** (résolveur ensembliste) ; gap « révocation » modélisé (fait séparé). **Reste, à l'écriture du 1er PROMO** : gap « id à source unique » (garde `promo/`) |
| **D-023** | **La fenêtre active** — champ `Status:Draft` d'OCR-000/005 lu et conséquent (noindex, `lifecycle_status` Draft, sitemap, garde:132) alors que RATIF-001 les déclare Normative | **✅ RENDUE le 2026-07-25 — voie C (divergence tolérée)** : **Q1** enjeu nul (site non lancé) → **tolérée + documentée** (dette bornée, `DETTES-ouvertes.md`) ; **Q2** la garde `recordPage.test` **évolue maintenant** — exception **nominative {OCR-000, OCR-005}** (= ensemble ratifié, prouvé par mutation), **levée à l'étape 6**. Les 5 lecteurs notés pour le résolveur · **✅ CLOSE le 2026-07-26 (étape 6, `d36ae71`)** : résolveur `resolveStatus` construit, les 3 familles dérivent du fait, `{000,005}` → Normative/index, exception `:132` **retirée** (palier 4), dette bornée **close** (`DETTES-ouvertes.md`) |
...
| **1** | **Amender OCR-006** (édition en place, D-018 : Version **reste 1.0.0**) — portée protocole (**C/E/G**), aligner A/B, corriger §295/§291 — **puis PROMOUVOIR** (Draft non opposable) | D-014 · D-011 · D-018 · D-021 · `OCR-000:47` | ✅ **1a FAIT** (`bfc888a`) — OCR-006 amendé (portée corrigée), diff périmètre exact, Version inchangée, manifeste régénéré (checksum OCR-006), `citations` re-mesuré (000/005→7). **Place dans l'ordre fixée (D-021, 2ᵉ).** 1b : **éditorial** (pas de grounding), **promotion effective → étape 6/8** |
...
| **5** | ~~**Grounder les 6** : `101,104,106,107,108,109`~~ **⚠️ liste PÉRIMÉE (F)** — les 32 sont **déjà grounés** (OCR-GROUND-001) ; la catégorie réelle est **11 machine-facing G1-dominant** (cœur Conforme absent) : `105,106,107,108,109,117,118,119,121,122,123`. Leur sort dépend de **E** (seuil « prêt » d'un G1), ouverte. Réf. [[RANGEMENT-B-D-F-G-conception]] | D-017 · **E** · `OCR-000:49-53` | ⛔ non lancée |
| **6** | **Concevoir l'artefact PROMO** — champ `authority` ; présuppose le grounding ; **rouvre D-010** ; **+ interface de projection au résolveur** (voie 2, `declaredRecords`) **+ forme de révocation COMMUNE** PROMO⁄RATIF (décidée **une fois**, D-013) | D-001 · **D-010** · D-016 · D-013 | 🛠️ **MÉCANISME CONSTRUIT** (SPEC-ETAPE6-resolveur) — résolveur `resolveStatus` (`4bf8977`) + garde anti-forgeage (`123c0ca`) + migration des 5 lecteurs (`d36ae71`) + fermeture D-023 (palier 4). Faits typés (RATIF/PROMO/révocation), `loadFacts`, révocation commune D-013. **L'ÉMISSION d'un `PROMO` reste l'étape 8** (acte distinct, non fait) ; **D-010 RÉSOLUE (grain souple, 2026-07-26)** — le résolveur l'accepte déjà ; le **lieu `promo/` + sa garde** (acceptant 1+ ids) restent à créer avec le 1er PROMO |
| **7** | **Normaliser les 36** — par **amendements** (OCR-000…005, `P10:280`), **pas** un Record neuf | D-015 · `P10:280` | ⛔ non lancée |
| **8** | **Promouvoir la Phase 1 = 32** `Draft → Normative` (dont `{OCR-000, OCR-005}` ratifiés en 0b) **dans l'ordre D-021 EFFECTIF** : `{OCR-000, OCR-005}` → `OCR-006` → `{001, 002, 003, 004}` → `{007, 008, 009}` → **22 concepts** (= 23 − `OCR-112`, sorti en Phase 2 par **D-024**). Ordre effectif = **32** = Phase 1 (écart en extension = **vide**, mesuré). Garde d'invariant d'ordre (D-020, faisable `4ede7f7`) à **étendre aux strates** | **D-021** · **D-024** · D-020 · D-016 · `OCR-000:47` | ⛔ non lancée · *sync aval faite (récitation « 29 » / « 23 concepts » périmée corrigée)* |
